...
def index(request):
    ...
    try:
        ...
        if not (os.path.exists(voltage_model_path) and os.path.exists(current_model_path) and os.path.exists(feature_columns_path)):
            ...
            predicted_bill = (predicted_current * predicted_voltage * 720) / 1000 / 16.45
            predicted_bills = round(predicted_bill, 2)
    except Exception as e:
...
def predict_bill(request):
    from AI_models.trainer import train_and_save_models
    readings = ElectricityReading.objects.all()
    prediction = train_and_save_models(readings)
